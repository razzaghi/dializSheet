        ...
        <div class="portlet box green">
            ...
            <div class="portlet-body">
                <table class="table table-striped table-hover table-responsive datatable" id="datatable">
                    ...
                    <tbody>
                    @foreach ($doctororder as $row)
                        <tr>
                            <td>
                                {!! Form::checkbox('del-'.$row->id,1,false,['class' => 'single','data-id'=> $row->id]) !!}
                            </td>
                            <td>{{ isset($row->patient->family) ? $row->patient->family : '' }}</td>
                            <td>{{ $row->dateTimeOrder }}</td>
                            <td>{{ isset($row->doctor->family) ? $row->doctor->family : '' }}</td>
                            <td>{{ isset($row->nurse->family) ? $row->nurse->family : '' }}</td>

                            <td>
                                {!! link_to_route('admin.doctororder.edit', trans('quickadmin::templates.templates-view_index-edit'), array($row->id), array('class' => 'btn btn-xs btn-info')) !!}
                                {!! Form::open(array('style' => 'display: inline-block;', 'method' => 'DELETE', 'onsubmit' => "return confirm('".trans("quickadmin::templates.templates-view_index-are_you_sure")."');",  'route' => array('admin.doctororder.destroy', $row->id))) !!}
